			   ...
			   const char *r_name)
{
  ...
  table_size = sizeof (elf_howto_table) / sizeof (elf_howto_table[0]);
...
spu_elf_setup (struct bfd_link_info *info, struct spu_elf_params *params)
{
  ...
  if (htab->params->max_branch > 0) {
    bfd_vma max_branch_log2 = bfd_log2 (htab->params->max_branch);
    htab->fromelem_size_log2 = (max_branch_log2 > 4) ? (max_branch_log2 - 4) : 0;
  } else {
    htab->fromelem_size_log2 = 0;
  }
}
  ...
  if (r_symndx >= symtab_hdr->sh_info)
    {
      ...
      unsigned long hash_index = r_symndx - symtab_hdr->sh_info;
      struct elf_link_hash_entry *h = sym_hashes[hash_index];
...
spu_elf_create_sections (struct bfd_link_info *info)
{
  ...
  if (ibfd == NULL)
    {
      if (!create_spu_note_section(info))
        return false;
    }

  if (htab->params && htab->params->emit_fixups)
    {
...
static bool
create_spu_note_section(struct bfd_link_info *info)
{
  ...
  size_t aligned_plugin_size;
  size_t aligned_name_size;
  ...
  if (!s || !bfd_set_section_alignment(s, 4))
    return false;

  elf_section_type(s) = SHT_NOTE;

  name_len = strlen(output_filename) + 1;
  plugin_name_size = sizeof(SPU_PLUGIN_NAME);
  aligned_plugin_size = (plugin_name_size + 3) & ~3;
  aligned_name_size = (name_len + 3) & ~3;
  size = 12 + aligned_plugin_size + aligned_name_size;
  ...
  bfd_put_32(ibfd, plugin_name_size, data);
  bfd_put_32(ibfd, name_len, data + 4);
  bfd_put_32(ibfd, 1, data + 8);
  memcpy(data + 12, SPU_PLUGIN_NAME, plugin_name_size);
  memcpy(data + 12 + aligned_plugin_size, output_filename, name_len);
...
create_fixup_section(struct spu_link_hash_table *htab, struct bfd_link_info *info)
{
  ...
  flagword flags = SEC_LOAD | SEC_ALLOC | SEC_READONLY | SEC_HAS_CONTENTS |
...
static int
process_soft_icache_overlays(struct bfd_link_info *info, struct spu_link_hash_table *htab,
                            asection **alloc_sec, unsigned int n,
                            unsigned int *ovl_index, unsigned int *num_buf,
                            bfd_vma *ovl_end)
{
    unsigned int i;
...
static int
process_regular_overlays(struct bfd_link_info *info, struct spu_link_hash_table *htab,
                        asection **alloc_sec, unsigned int n,
                        unsigned int *ovl_index, unsigned int *num_buf,
                        bfd_vma *ovl_end)
{
...
is_indirect_branch (const unsigned char *insn)
{
  ...
  const unsigned char MASK_FIRST_BYTE = 0xef;
  const unsigned char EXPECTED_FIRST = 0x25;
  const unsigned char MASK_SECOND_BYTE = 0x80;
  const unsigned char EXPECTED_SECOND = 0x00;
  
  return (insn[0] & MASK_FIRST_BYTE) == EXPECTED_FIRST && 
         (insn[1] & MASK_SECOND_BYTE) == EXPECTED_SECOND;
}
		...
		struct bfd_link_info *info)
{
  ...
  if (r_type == R_SPU_REL16 || r_type == R_SPU_ADDR16)
	...
	{
	  content_ptr = contents + irela->r_offset;
	}

      branch = is_branch (content_ptr);
      hint = is_hint (content_ptr);
      if (branch || hint)
	{
	  call = (content_ptr[0] & 0xfd) == 0x31;
	  if (call && sym_type != STT_FUNC && content_ptr != insn)
	    {
	      const char *sym_name;
	    ...
	    const Elf_Internal_Rela *irela)
{
  ...
  if (htab == NULL || ibfd == NULL || isec == NULL)
    return false;

  if (stub_type != nonovl_stub)
    ovl = spu_elf_section_data (isec->output_section)->u.o.ovl_index;

  head = get_got_entry_head(h, ibfd, irela);
  if (head == NULL)
    return false;
  ...
  g = find_matching_entry(*head, addend, ovl);
  
  if (g == NULL)
    {
      if (ovl == 0)
        {
          remove_matching_entries(head, addend, htab);
        }
      
      g = create_new_entry(addend, ovl);
      if (g == NULL)
        return false;
...
get_got_entry_head(struct elf_link_hash_entry *h, bfd *ibfd, const Elf_Internal_Rela *irela)
{
  if (h != NULL)
    return &h->got.glist;
...
static struct got_entry *
find_matching_entry(struct got_entry *head, bfd_vma addend, unsigned int ovl)
{
  struct got_entry *g;
...
static void
remove_matching_entries(struct got_entry **head, bfd_vma addend, struct spu_link_hash_table *htab)
{
  struct got_entry *g, *gnext, *prev;
...
static struct got_entry *
create_new_entry(bfd_vma addend, unsigned int ovl)
{
...
ovl_stub_size (struct spu_elf_params *params)
{
  ...
  unsigned int flavour_multiplier = 1U << params->ovly_flavour;
  unsigned int compact_divisor = 1U << params->compact_stub;
  
  return (base_size * flavour_multiplier) / compact_divisor;
}
...
ovl_stub_size_log2 (struct spu_elf_params *params)
{
  ...
  return 4U + (unsigned int)params->ovly_flavour - (unsigned int)params->compact_stub;
}
	    ...
	    asection *dest_sec)
{
  ...
  head = (h != NULL) ? &h->got.glist : (elf_local_got_ents (ibfd) + ELF32_R_SYM (irela->r_info));
...
allocate_spuear_stubs(struct elf_link_hash_entry *h, void *inf)
{
    ...
    struct spu_link_hash_table *htab;
    asection *sym_sec;
    struct spu_elf_section_data *section_data;
    ...
    if (!h->root.root.string || !startswith(h->root.root.string, "_SPUEAR_")) {
        return true;
    }
    ...
    section_data = spu_elf_section_data(sym_sec->output_section);
    if (!section_data) {
        return true;
    }

    if (section_data->u.o.ovl_index == 0 && !htab->params->non_overlay_stubs) {
        return true;
    }
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
    {
      ...
      for (isec = ibfd->sections; isec != NULL; isec = isec->next)
        {
          if (!process_section_stubs(info, htab, ibfd, isec, symtab_hdr, 
                                   &local_syms, build))
            {
              cleanup_local_syms(symtab_hdr, local_syms, info);
              return false;
            }
        }
                     ...
                     bool build)
{
  ...
  for (; irela < irelaend; irela++)
    {
      if (!process_relocation(info, htab, ibfd, isec, irela, 
                            symtab_hdr, local_syms, build))
        {
          cleanup_internal_relocs(isec, internal_relocs);
          return false;
        }
    }
...
static bool
process_relocation(struct bfd_link_info *info,
                  struct spu_link_hash_table *htab,
                  bfd *ibfd, asection *isec,
                  Elf_Internal_Rela *irela,
                  Elf_Internal_Shdr *symtab_hdr,
                  ...
                  bool build)
{
  ...
  if (build)
    return build_stub_for_relocation(info, ibfd, isec, stub_type, h, 
                                   irela, sym, sym_sec);
  else
...
static bool
build_stub_for_relocation(struct bfd_link_info *info,
                         bfd *ibfd, asection *isec,
                         enum _stub_type stub_type,
                         struct elf_link_hash_entry *h,
                         Elf_Internal_Rela *irela,
...
static void
cleanup_internal_relocs(asection *isec, Elf_Internal_Rela *internal_relocs)
{
...
static void
cleanup_local_syms(Elf_Internal_Shdr *symtab_hdr, 
                  Elf_Internal_Sym *local_syms,
                  struct bfd_link_info *info)
{
  if (local_syms == NULL)
...
spu_elf_place_overlay_data (struct bfd_link_info *info)
{
  ...
  if (htab->stub_sec != NULL)
    {
      if (htab->params != NULL && htab->params->place_spu_section != NULL)
        {
  ...
  if (htab->params != NULL && htab->params->place_spu_section != NULL)
    {
      ...
      if (htab->ovtab != NULL)
        {
          ovout = (htab->params->ovly_flavour == ovly_soft_icache) ? ".bss" : ".data";
...
static void *
ovl_mgr_open (struct bfd *nbfd, void *stream)
{
  (void)nbfd;
  return stream;
}
	       ...
	       file_ptr offset)
{
  ...
  if (!stream || !buf || nbytes < 0 || offset < 0)
    return 0;

  os = (struct _ovl_stream *) stream;
  
  if (!os->start || !os->end || os->start > os->end)
    return 0;
...
spu_elf_build_stubs (struct bfd_link_info *info)
{
  ...
  if (htab->params->ovly_flavour == ovly_soft_icache)
  ...
  else
    {
      ...
      if (!define_overlay_symbols(htab))
        return false;
    }
...
static bool
define_icache_symbols(struct spu_link_hash_table *htab)
{
  struct elf_link_hash_entry *h;
  bfd_vma off = 0;
  ...
  if (h == NULL) return false;
  h->root.u.def.value = 16 << (htab->fromelem_size_log2 + htab->num_lines_log2);
  h->root.u.def.section = bfd_abs_section_ptr;
...
static bool
define_overlay_symbols(struct spu_link_hash_table *htab)
{
  struct elf_link_hash_entry *h;
...
spu_elf_check_vma (struct bfd_link_info *info)
{
  ...
  bfd *abfd;
  bfd_vma hi, lo;
  bfd_vma section_start, section_end;
  ...
  for (m = elf_seg_map (abfd); m != NULL; m = m->next) {
    ...
    for (i = 0; i < m->count; i++) {
      if (!m->sections[i] || m->sections[i]->size == 0)
        continue;

      section_start = m->sections[i]->vma;
      section_end = section_start + m->sections[i]->size - 1;

      if (section_start < lo || section_start > hi || section_end > hi)
        return m->sections[i];
    }
  }
...
{
  Elf_Internal_Sym *const *s1 = (Elf_Internal_Sym *const *)a;
  Elf_Internal_Sym *const *s2 = (Elf_Internal_Sym *const *)b;
  asection *sec1, *sec2;
...
alloc_stack_info (asection *sec, int max_fun)
{
  ...
  if (max_fun > (SIZE_MAX - sizeof (struct spu_elf_stack_info)) / sizeof (struct function_info))
    return NULL;
  ...
  if (sinfo->num_fun >= sinfo->max_fun)
    {
      ...
      new_max_fun = sinfo->max_fun + 20 + (sinfo->max_fun >> 1);
      if (new_max_fun <= sinfo->max_fun)
	return NULL;
      amt += (new_max_fun - 1) * sizeof (struct function_info);
      sinfo = (struct spu_elf_stack_info *)bfd_realloc (sinfo, amt);
      if (sinfo == NULL)
	return NULL;
      memset ((char *) sinfo + old, 0, amt - old);
      sinfo->max_fun = new_max_fun;
...
func_name(struct function_info *fun)
{
    ...
    if (fun->u.sym->st_name == 0)
    {
        ...
        char *name = bfd_malloc(len + 20);
        if (name == NULL)
            return "(null)";
        sprintf(name, "%s+%lx", sec->name,
               (unsigned long)(fun->u.sym->st_value & 0xffffffffUL));
        return name;
    }
...
interesting_section (asection *s)
{
  ...
  const bfd_vma required_flags = SEC_ALLOC | SEC_LOAD | SEC_CODE;
  const bfd_vma masked_flags = s->flags & (SEC_ALLOC | SEC_LOAD | SEC_CODE | SEC_IN_MEMORY);
  
  return masked_flags == required_flags;
}
...
discover_functions (struct bfd_link_info *info)
{
  ...
  for (ibfd = info->input_bfds, bfd_idx = 0;
	...
	continue;
      ...
      if (!process_bfd_symbols(ibfd, bfd_idx, psym_arr, sec_arr, &gaps)) {
        result = false;
        goto cleanup;
      }
    }
  ...
  if (gaps) {
    if (!process_gaps(info, psym_arr, sec_arr, bfd_count)) {
      result = false;
      goto cleanup;
    }
...
cleanup:
  for (bfd_idx = 0; bfd_idx < bfd_count; bfd_idx++) {
...
static bool
process_bfd_symbols(bfd *ibfd, int bfd_idx, Elf_Internal_Sym ***psym_arr, 
                   asection ***sec_arr, bool *gaps)
{
  ...
  if (!filter_and_sort_symbols(syms, symcount, psyms, psecs, &symcount))
    return false;

  if (!allocate_stack_info(psyms, psecs, syms, symcount))
    return false;

  if (!install_function_symbols(psyms, psecs, syms, symcount))
    return false;
...
static bool
filter_and_sort_symbols(Elf_Internal_Sym *syms, size_t symcount,
...
static bool
allocate_stack_info(Elf_Internal_Sym **psyms, asection **psecs,
                    Elf_Internal_Sym *syms, size_t symcount)
{
...
static bool
install_function_symbols(Elf_Internal_Sym **psyms, asection **psecs,
                        Elf_Internal_Sym *syms, size_t symcount)
{
  Elf_Internal_Sym **psy;
...
process_gaps(struct bfd_link_info *info, Elf_Internal_Sym ***psym_arr,
             asection ***sec_arr, int bfd_count)
{
  bfd *ibfd;
  int bfd_idx;
  ...
  return process_global_symbols(info, psym_arr, sec_arr, bfd_count);
}

static bool
process_global_symbols(struct bfd_link_info *info, Elf_Internal_Sym ***psym_arr,
                       asection ***sec_arr, int bfd_count)
{
  bfd *ibfd;
  int bfd_idx;
  ...
  for (ibfd = info->input_bfds, bfd_idx = 0;
    ...
    Elf_Internal_Sym *syms, *sy, **psyms, **psy;
    asection **psecs;
    bool local_gaps = false;

    psyms = psym_arr[bfd_idx];
    if (psyms == NULL)
      continue;
    ...
    for (sec = ibfd->sections; sec != NULL && !local_gaps; sec = sec->next) {
      if (interesting_section (sec))
        local_gaps |= check_function_ranges (sec, info);
    }
...
extend_function_ranges(struct bfd_link_info *info)
{
  ...
  for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next) {
    ...
    for (sec = ibfd->sections; sec != NULL; sec = sec->next) {
      if (interesting_section (sec)) {
        if (!extend_section_function_ranges(sec))
          return false;
      }
    }
  }
...
static bool
extend_section_function_ranges(asection *sec)
{
  struct _spu_elf_section_data *sec_data;
  struct spu_elf_stack_info *sinfo;
      ...
      if (call->fun)
        {
          ...
          if (!mark_non_root (call->fun, info, param))
            return false;
        }
    }
  return true;
	       ...
	       void *param)
{
  ...
  unsigned int depth = *depth_param;
  ...
  while ((call = *callp) != NULL)
      ...
      else if (call->fun->marking)
	{
	  ...
	  if (htab != NULL && !htab->params->auto_overlay
	      && htab->params->stack_analysis)
	    {
	      const char *f1 = func_name (fun);
	      const char *f2 = func_name (call->fun);

	      if (f1 != NULL && f2 != NULL && info->callbacks != NULL && info->callbacks->info != NULL)
		{
		      ...
		      void *param)
{
  ...
  if (should_mark_section(fun, htab))
    {
      if (!process_section_marking(fun, htab, mos_param))
        return false;
    }
  ...
  handle_entry_code_exclusion(fun, info);
  return true;
}
...
static bool
process_section_marking(struct function_info *fun, struct spu_link_hash_table *htab, 
...
static bool
process_rodata_section(struct function_info *fun, struct spu_link_hash_table *htab,
                      unsigned int *size)
{
  char *name = get_rodata_name(fun->sec->name);
  if (name == NULL)
    return false;

  asection *rodata = find_rodata_section(fun, name);
  fun->rodata = rodata;
  
  if (fun->rodata)
    {
      unsigned int new_size = *size + fun->rodata->size;
...
static char *
get_rodata_name(const char *sec_name)
{
  char *name = NULL;
  size_t len;
...
static asection *
find_rodata_section(struct function_info *fun, const char *name)
{
  asection *group_sec = elf_section_data (fun->sec)->next_in_group;
  
  if (group_sec == NULL)
    return bfd_get_section_by_name (fun->sec->owner, name);
...
static void
handle_entry_code_exclusion(struct function_info *fun, struct bfd_link_info *info)
{
			...
			void *param)
{
  ...
  excluded = ((fun->sec == uos_param->exclude_input_section) ||
              (fun->sec != NULL && fun->sec->output_section == uos_param->exclude_output_section)) ? 1 : 0;
  ...
  if (size <= lib_param->lib_size)
    {
      *lib_param->lib_sections = fun->sec;
      ...
      if (fun->rodata != NULL && fun->rodata->linker_mark && fun->rodata->gc_mark)
	{
...
sort_lib (const void *a, const void *b)
{
  asection *const *s1 = a;
  asection *const *s2 = b;
  struct _spu_elf_section_data *sec_data1;
  struct _spu_elf_section_data *sec_data2;
  struct spu_elf_stack_info *sinfo1;
  struct spu_elf_stack_info *sinfo2;
  int delta1 = 0;
  int delta2 = 0;
  int i;

  sec_data1 = spu_elf_section_data (*s1);
  if (sec_data1 != NULL)
    {
      sinfo1 = sec_data1->u.i.stack_info;
      if (sinfo1 != NULL)
        {
          for (i = 0; i < sinfo1->num_fun; ++i)
            delta1 += sinfo1->fun[i].call_count;
        }
    }

  sec_data2 = spu_elf_section_data (*s2);
  if (sec_data2 != NULL)
    {
      sinfo2 = sec_data2->u.i.stack_info;
      if (sinfo2 != NULL)
        {
          for (i = 0; i < sinfo2->num_fun; ++i)
            delta2 += sinfo2->fun[i].call_count;
        }
    }
...
auto_ovl_lib_functions (struct bfd_link_info *info, unsigned int lib_size)
{
  ...
  for (i = 0; i < lib_count; i++) {
        ...
        if (sinfo != NULL) {
          ...
          for (k = 0; k < sinfo->num_fun; k++) {
            for (call = sinfo->fun[k].call_list; call; call = call->next) {
              if (call->fun && call->fun->sec && call->fun->sec->linker_mark) {
                struct call_info *p;
    ...
    if (tmp + stub_size < lib_size) {
      ...
      while ((p = *pp) != NULL) {
        if (p->fun && p->fun->sec && !p->fun->sec->linker_mark) {
              ...
              if (call->fun && call->fun->sec && call->fun->sec->linker_mark) {
                ...
                if (callee == NULL) {
                  while (dummy_caller.call_list != NULL) {
                    struct call_info *temp_call = dummy_caller.call_list;
                    dummy_caller.call_list = temp_call->next;
                    free (temp_call);
                  }
                  free(lib_sections);
                  return (unsigned int) -1;
		  ...
		  void *param)
{
  ...
  if (!fun->sec || !fun->sec->linker_mark || !fun->sec->gc_mark)
    goto process_remaining_calls;
  ...
  *(*ovly_sections)++ = fun->sec;
  ...
  if (fun->sec->segment_mark)
      ...
      do
	{
      ...
      while (call_fun->sec && call_fun->sec->segment_mark);
    }

process_remaining_calls:
  for (call = fun->call_list; call != NULL; call = call->next)
    if (!call->broken_cycle
...
static const char*
get_archive_filename(asection *sec)
{
...
process_pasted_calls(FILE *script, asection *sec, struct bfd_link_info *info, int use_rodata)
{
  ...
  while (call != NULL) {
    ...
    call = call_fun->call_list;
    while (call && !call->is_pasted) {
      call = call->next;
    }
  }
                          ...
                          struct bfd_link_info *info)
{
  ...
  for (j = base; j < count && ovly_map[j] == ovlynum; j++) {
    ...
    if (sec && process_pasted_calls(script, sec, info, 1) < 0) {
      return -1;
    }
  }
...
spu_elf_auto_overlay (struct bfd_link_info *info)
{
  ...
  if (bfd_count > 1) {
    ...
    for (i = 1; i < bfd_count; ++i) {
      const char *prev_name = bfd_get_filename (bfd_arr[i - 1]);
      const char *curr_name = bfd_get_filename (bfd_arr[i]);
      
      if (prev_name && curr_name && filename_cmp (prev_name, curr_name) == 0) {
  ...
  if (fixed_size + mos_param.max_overlay_size <= htab->local_store) {
    if (htab->params->ovly_flavour == ovly_soft_icache) {
      ...
      if (htab->local_store > fixed_size) {
  ...
  while (base < count) {
    ...
    for (i = base; i < count; i++) {
          ...
          for (k = 0; k < (unsigned) sinfo->num_fun; ++k) {
            for (call = sinfo->fun[k].call_list; call; call = call->next) {
              if (call->is_pasted) {
                ...
                pasty = call;
              } else if (call->fun && call->fun->sec && call->fun->sec->linker_mark) {
      ...
      for (call = dummy_caller.call_list; call; call = call->next) {
        ...
        for (k = base; k < i + 1; k++) {
          if (call->fun && call->fun->sec == ovly_sections[2 * k]) {
            num_stubs -= stub_delta;
            break;
          }
        }
    ...
    if (i == base) {
      if (ovly_sections[2 * i] && ovly_sections[2 * i]->owner) {
...
spu_elf_stack_analysis (struct bfd_link_info *info)
{
  ...
  if (htab->params->stack_analysis && info->callbacks && info->callbacks->info && info->callbacks->minfo)
    {
...
spu_elf_final_link (bfd *output_bfd, struct bfd_link_info *info)
{
  ...
  bool needs_analysis = htab->params->stack_analysis ||
                        (htab->params->ovly_flavour == ovly_soft_icache &&
      ...
      if (r_symndx < symtab_hdr->sh_info)
      ...
      else
        {
          ...
          if (h->root.type == bfd_link_hash_defined ||
            ...
            {
              /* Handled by default relocation = 0 */
            }
...
static bool
spu_elf_finish_dynamic_sections (bfd *output_bfd,
				 struct bfd_link_info *info)
{
  (void)output_bfd;
  (void)info;
			    ...
			    struct elf_link_hash_entry *h)
{
  ...
  if (bfd_link_relocatable (info) || h == NULL)
    return 1;
  ...
  if (!h->def_regular || !startswith (h->root.root.string, "_SPUEAR_"))
    return 1;
...
spu_elf_modify_segment_map (bfd *abfd, struct bfd_link_info *info)
{
  ...
  for (m = elf_seg_map (abfd); m != NULL; m = m->next)
  {
    ...
    for (i = 0; i < m->count; i++)
    {
      ...
      if (i + 1 < m->count)
      {
        ...
        bfd_vma amt = sizeof (struct elf_segment_map) + (m->count - (i + 2)) * sizeof (m->sections[0]);
  ...
  while (*p != NULL)
  {
    if ((*p)->p_type == PT_LOAD)
    {
      ...
      if ((*p)->count == 1 && spu_elf_section_data ((*p)->sections[0])->u.o.ovl_index != 0)
      {
        m = *p;
        m->no_sort_lma = 1;
        *p = m->next;
...
spu_elf_modify_headers (bfd *abfd, struct bfd_link_info *info)
{
  ...
  if (tdata == NULL || tdata->phdr == NULL || bed == NULL || bed->s == NULL)
    return _bfd_elf_modify_headers (abfd, info);
  ...
  if (htab->num_overlays != 0)
    {
      ...
      for (m = elf_seg_map (abfd); m != NULL; m = m->next, i++)
        {
          if (m->count == 0 || m->sections[0] == NULL)
            continue;
          ...
          if (htab->ovtab != NULL && htab->ovtab->size != 0
              && htab->params != NULL && htab->params->ovly_flavour != ovly_soft_icache)
            {
      ...
      if (htab->init != NULL && htab->init->size != 0 && htab->ovl_sec != NULL)
        {
  ...
  for (unsigned int i = count; i > 0; i--)
    {
      ...
      unsigned int filesz_adjust = (-phdr[idx].p_filesz) & 15;
      if (filesz_adjust != 0 && last != NULL
          && (phdr[idx].p_offset + phdr[idx].p_filesz > last->p_offset - filesz_adjust))
        {
          can_adjust = false;
          break;
        }

      unsigned int memsz_adjust = (-phdr[idx].p_memsz) & 15;
      if (memsz_adjust != 0 && last != NULL && phdr[idx].p_filesz != 0
          && phdr[idx].p_vaddr + phdr[idx].p_memsz > last->p_vaddr - memsz_adjust
      ...
      for (unsigned int i = count; i > 0; i--)
        {
          ...
          unsigned int filesz_adjust = (-phdr[idx].p_filesz) & 15;
          phdr[idx].p_filesz += filesz_adjust;

          unsigned int memsz_adjust = (-phdr[idx].p_memsz) & 15;
          phdr[idx].p_memsz += memsz_adjust;
        }
    }
...
{
  struct spu_link_hash_table *htab = spu_hash_table (info);
  if (!htab || !htab->params || !htab->params->emit_fixups)
    return true;
