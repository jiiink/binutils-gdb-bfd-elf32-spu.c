...
static reloc_howto_type *spu_elf_reloc_type_lookup(bfd *abfd ATTRIBUTE_UNUSED, bfd_reloc_code_real_type code) {
  ...
  if (r_type < 0 || r_type >= (enum elf_spu_reloc_type)NUM_ELEM(elf_howto_table))
    return NULL;
...
{
    size_t count = sizeof(elf_howto_table) / sizeof(elf_howto_table[0]);
    size_t i;
    ...
    for (i = 0; i < count; ++i) {
        const char *name = elf_howto_table[i].name;
             ...
             bfd *output_bfd, char **error_message)
{
    ...
    if (!bfd_is_com_section(symbol->section))
        val = symbol->value;
    if (symbol->section && symbol->section->output_section)
        val += symbol->section->output_section->vma;
    val += reloc_entry->addend;

    if (!input_section || !input_section->output_section)
        return bfd_reloc_outofrange;
    ...
    bfd_vma encoded_val = (val & 0x7f) | ((val & 0x180) << 7) | ((val & 0x180) << 16);
    insn = (insn & ~reloc_entry->howto->dst_mask) | (encoded_val & reloc_entry->howto->dst_mask);
...
static bool spu_elf_object_p(bfd *abfd) {
    ...
    unsigned int num_ovl = 0, num_buf = 0;
        ...
        if (phdr->p_type == PT_LOAD && (phdr->p_flags & PF_OVERLAY) != 0) {
            ...
            for (unsigned int j = 1; j < num_sections; j++) {
                ...
                if (shdr->bfd_section &&
                    ...
                    spu_elf_section_data(shdr->bfd_section)->u.o.ovl_index = num_ovl;
                    spu_elf_section_data(shdr->bfd_section)->u.o.ovl_buf = num_buf;
                }
            }
        }
    }
...
{
    if (sym == NULL || sym->name == NULL || sym->section == bfd_abs_section_ptr) {
        return;
    }
...
spu_elf_link_hash_table_create(bfd *abfd)
{
    ...
    memset(&htab->elf.init_got_refcount, 0, sizeof(htab->elf.init_got_refcount));
    memset(&htab->elf.init_got_offset, 0, sizeof(htab->elf.init_got_offset));
...
static bool get_sym_h(struct elf_link_hash_entry **hp, Elf_Internal_Sym **symp, asection **symsecp, Elf_Internal_Sym **locsymsp, unsigned long r_symndx, bfd *ibfd) {
    ...
    if (r_symndx >= symtab_hdr->sh_info) {
        ...
        if (sym_hashes == NULL)
            return false;
        size_t idx = r_symndx - symtab_hdr->sh_info;
        struct elf_link_hash_entry *h = sym_hashes[idx];
        while (h != NULL &&
        ...
        if (locsyms == NULL) {
            ...
            if (locsyms == NULL) {
                locsyms = bfd_elf_get_elf_syms(ibfd, symtab_hdr, symtab_hdr->sh_info, 0, NULL, NULL, NULL);
                if (locsyms == NULL)
                    return false;
                *locsymsp = locsyms;
            }
...
bool spu_elf_create_sections(struct bfd_link_info *info) {
    ...
    if (!info || !htab || !htab->params)
        return false;

    for (ibfd = info->input_bfds; ibfd; ibfd = ibfd->link.next) {
        if (bfd_get_section_by_name(ibfd, SPU_PTNOTE_SPUNAME))
            break;
    }

    if (!ibfd) {
        asection *s;
        size_t name_len, size, plugin_len, aligned_plugin_len, aligned_name_len;
        bfd_byte *data;
        flagword flags;
        ...
        plugin_len = sizeof(SPU_PLUGIN_NAME);
        name_len = strlen(bfd_get_filename(info->output_bfd)) + 1;
        aligned_plugin_len = (plugin_len + 3) & ~3U;
        aligned_name_len = (name_len + 3) & ~3U;

        size = 12 + aligned_plugin_len + aligned_name_len;
        ...
        bfd_put_32(ibfd, plugin_len, data);
        bfd_put_32(ibfd, name_len, data + 4);
        bfd_put_32(ibfd, 1, data + 8);

        memcpy(data + 12, SPU_PLUGIN_NAME, plugin_len);
        memset(data + 12 + plugin_len, 0, aligned_plugin_len - plugin_len);
        memcpy(data + 12 + aligned_plugin_len, bfd_get_filename(info->output_bfd), name_len);
        memset(data + 12 + aligned_plugin_len + name_len, 0, aligned_name_len - name_len);
    ...
    if (htab->params->emit_fixups) {
        ...
        flags = SEC_LOAD | SEC_ALLOC | SEC_READONLY | SEC_HAS_CONTENTS | SEC_IN_MEMORY | SEC_LINKER_CREATED;
...
int spu_elf_find_overlays(struct bfd_link_info *info) {
    ...
    unsigned int i = 0, n = 0, ovl_index = 0, num_buf = 0;
    asection *s = NULL;
    bfd_vma ovl_end = 0;
    ...
    /* Gather all allocated, loadable, non-thread-local, nonzero sections */
    for (s = info->output_bfd->sections, n = 0; s; s = s->next) {
...
{
    if (!input_section || (input_section->flags & SEC_ALLOC) == 0)
        return false;
    if (input_section->output_section == bfd_abs_section_ptr)
        return false;
    if (input_section->name && strcmp(input_section->name, ".eh_frame") == 0)
        return false;
    return true;
}
...
static enum _stub_type needs_ovl_stub(struct elf_link_hash_entry *h,
                                      Elf_Internal_Sym *sym,
                                      asection *sym_sec,
                                      ...
                                      struct bfd_link_info *info)
{
    ...
    if (r_type == R_SPU_REL16 || r_type == R_SPU_ADDR16) {
        if (!contents) {
            if (!bfd_get_section_contents(input_section->owner, input_section, insn, irela->r_offset, 4)) {
                return stub_error;
            }
            contents = insn;
        } else {
        ...
        spu_elf_section_data(input_section->output_section)->u.o.ovl_index)
    {
        ...
        if (!lrlive && (call || sym_type == STT_FUNC)) {
            ...
            ret = (enum _stub_type)(br000_ovl_stub + lrlive);
        }
    }
...
{
    unsigned int ovl = 0;
    struct got_entry *g = NULL, **head = NULL;
    bfd_vma addend = 0;
    bfd_size_type amt;
    ...
    if (h != NULL) {
        ...
        if (elf_local_got_ents(ibfd) == NULL) {
            amt = elf_tdata(ibfd)->symtab_hdr.sh_info * sizeof(*elf_local_got_ents(ibfd));
    ...
    if (ovl == 0) {
        struct got_entry *prev = NULL, *tmp = *head, *gnext;
        bool found = false;
        while (tmp != NULL) {
            gnext = tmp->next;
            if (tmp->addend == addend && tmp->ovl == 0) {
                g = tmp;
                found = true;
                break;
            }
        ...
        if (!found) {
            ...
            while (tmp != NULL) {
                gnext = tmp->next;
                if (tmp->addend == addend) {
                    htab->stub_count[tmp->ovl]--;
                    free(tmp);
                    if (prev)
                        prev->next = gnext;
                    else
...
static unsigned int ovl_stub_size(struct spu_elf_params *params)
{
    ...
    if (params->ovly_flavour > 0)
        size <<= params->ovly_flavour;
    if (params->compact_stub > 0)
        size >>= params->compact_stub;

    return size;
}

static unsigned int ovl_stub_size_log2(const struct spu_elf_params *params)
{
    if (!params) {
        return 0;
    }
    ...
    if (params->ovly_flavour > params->compact_stub) {
        return base + (params->ovly_flavour - params->compact_stub);
    }
    return base;
}
...
{
    struct spu_link_hash_table *htab = spu_hash_table(info);
    unsigned int ovl = 0, dest_ovl = 0, set_id = 0;
    struct got_entry *g = NULL, **head = NULL;
    asection *sec = NULL;
    bfd_vma addend = 0, from = 0, to = 0, br_dest = 0, patt = 0;
    unsigned int lrlive = 0;
    ...
    head = (h != NULL)
        ? &h->got.glist
        : elf_local_got_ents(ibfd) + ELF32_R_SYM(irela ? irela->r_info : 0);
    ...
    if (htab->params->ovly_flavour == ovly_normal && !htab->params->compact_stub) {
    ...
    } else if (htab->params->ovly_flavour == ovly_normal && htab->params->compact_stub) {
        if (!BRA_STUBS) {
        ...
        if (stub_type == nonovl_stub) {
            ...
            if (stub_type != br000_ovl_stub && lrlive != stub_type - br000_ovl_stub) {
                info->callbacks->einfo(
                    _("%pA:0x%v lrlive .brinfo (%u) differs from analysis (%u)\n"),
    ...
    if (htab->params->emit_stub_syms) {
        ...
        if (h != NULL) {
            ...
            sprintf(name + 8 + sizeof(".ovl_call.") - 1, "%x:%x",
                    ...
                    (int)ELF32_R_SYM(irela ? irela->r_info : 0) & 0xffffffff);
        }
        if (add != 0) {
...
static bool allocate_spuear_stubs(struct elf_link_hash_entry *h, void *inf)
{
        ...
        startswith(h->root.root.string, "_SPUEAR_"))
    {
        ...
        if (sym_sec &&
            sym_sec->output_section != bfd_abs_section_ptr)
        {
            void *section_data = spu_elf_section_data(sym_sec->output_section);
            if (section_data)
            {
                int ovl_index = ((struct elf_section_data *)section_data)->u.o.ovl_index;
                if (ovl_index != 0 || htab->params->non_overlay_stubs)
                {
...
static bool build_spuear_stubs(struct elf_link_hash_entry *h, void *inf)
{
    ...
    bool has_valid_root_type = (h->root.type == bfd_link_hash_defined || h->root.type == bfd_link_hash_defweak);
    bool has_def_regular = h->def_regular;
    bool has_spuear_prefix = startswith(h->root.root.string, "_SPUEAR_");
    sym_sec = (has_valid_root_type && has_def_regular && has_spuear_prefix) ? h->root.u.def.section : NULL;

    if (sym_sec && sym_sec->output_section && sym_sec->output_section != bfd_abs_section_ptr) {
        void *sec_data = spu_elf_section_data(sym_sec->output_section);
        if (sec_data != NULL) {
            int ovl_index = ((struct elf_section_data *)sec_data)->u.o.ovl_index;
            if (ovl_index != 0 || (htab && htab->params && htab->params->non_overlay_stubs)) {
...
process_stubs(struct bfd_link_info *info, bool build)
{
    ...
    for (ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next)
    {
        ...
        symtab_hdr = &elf_tdata(ibfd)->symtab_hdr;
        if (!symtab_hdr || symtab_hdr->sh_info == 0)
            continue;

        for (isec = ibfd->sections; isec != NULL; isec = isec->next)
        {
            Elf_Internal_Rela *internal_relocs = NULL, *irela = NULL, *irelaend = NULL;
            ...
            internal_relocs = _bfd_elf_link_read_relocs(ibfd, isec, NULL, NULL, info->keep_memory);
            if (!internal_relocs)
            {
                error_occurred = true;
                break;
            ...
            for (; irela < irelaend; irela++)
            {
                ...
                {
                    bfd_vma dest = (h ? h->root.u.def.value : sym->st_value) + irela->r_addend;
...
int spu_elf_size_stubs(struct bfd_link_info *info)
{
    ...
    if (htab->stub_count) {
        ...
        if (!stub || !bfd_set_section_alignment(stub, ovl_stub_size_log2(htab->params)))
            return 0;
    ...
    if (htab->params->ovly_flavour == ovly_soft_icache) {
        ...
        if (!htab->ovtab || !bfd_set_section_alignment(htab->ovtab, 4))
            return 0;
        ...
        if (!htab->init || !bfd_set_section_alignment(htab->init, 4))
            return 0;
    ...
    if (!htab->toe || !bfd_set_section_alignment(htab->toe, 4))
        return 0;
    htab->toe->size = 16;
...
void spu_elf_place_overlay_data(struct bfd_link_info *info)
{
    ...
    if (htab->stub_sec) {
        ...
        for (i = 0; i < htab->num_overlays; ++i) {
            asection *osec = htab->ovl_sec ? htab->ovl_sec[i] : NULL;
            if (!osec)
                continue;
            struct spu_elf_section_data_struct *sec_data = spu_elf_section_data(osec);
            if (!sec_data)
                continue;
            unsigned int ovl = sec_data->u.o.ovl_index;
            if (htab->stub_sec[ovl])
                htab->params->place_spu_section(htab->stub_sec[ovl], osec, NULL);
        }
    }

    if (htab->params->ovly_flavour == ovly_soft_icache && htab->init)
                             ...
                             file_ptr offset)
{
    ...
    max = (size_t)((const char *)os->end - (const char *)os->start);

    if ((ufile_ptr)offset >= max)
        return 0;

    if ((size_t)nbytes > max - (size_t)offset)
        count = max - (size_t)offset;
    else
        count = (size_t)nbytes;
...
static int ovl_mgr_stat(struct bfd *abfd ATTRIBUTE_UNUSED, void *stream, struct stat *sb) {
    if (!stream || !sb) {
        return -1;
    }
    ...
    if (!os->start || !os->end || os->end < os->start) {
        return -1;
    }
...
static unsigned int overlay_index(asection *sec) {
  if (!sec || sec->output_section == bfd_abs_section_ptr)
    return 0;

  spu_elf_section_data_type *data = spu_elf_section_data(sec->output_section);
  if (!data)
    return 0;
...
static bool spu_elf_build_stubs(struct bfd_link_info *info) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (h != NULL &&
                (h->root.type == bfd_link_hash_defined ||
                 h->root.type == bfd_link_hash_defweak) &&
                h->def_regular) {
                s = h->root.u.def.section ? h->root.u.def.section->output_section : NULL;
                if (s && spu_elf_section_data(s)->u.o.ovl_index) {
    ...
    if (htab->stub_sec != NULL) {
        for (i = 0; i <= htab->num_overlays; i++) {
            if (htab->stub_sec[i] && htab->stub_sec[i]->size != 0) {
    ...
    if (htab->params->ovly_flavour == ovly_soft_icache) {
        ...
        struct ovtab_sym {
            ...
            {"__icache_tag_array", 0, 16U << htab->num_lines_log2, NULL},
            {"__icache_tag_array_size", 16U << htab->num_lines_log2, 0, bfd_abs_section_ptr},
            {"__icache_rewrite_to", 0, 16U << htab->num_lines_log2, NULL},
            {"__icache_rewrite_to_size", 16U << htab->num_lines_log2, 0, bfd_abs_section_ptr},
            {"__icache_rewrite_from", 0, 16U << (htab->fromelem_size_log2 + htab->num_lines_log2), NULL},
            {"__icache_rewrite_from_size", 16U << (htab->fromelem_size_log2 + htab->num_lines_log2), 0, bfd_abs_section_ptr},
            {"__icache_log2_fromelemsize", htab->fromelem_size_log2, 0, bfd_abs_section_ptr},
            {"__icache_base", htab->ovl_sec && htab->ovl_sec[0] ? htab->ovl_sec[0]->vma : 0, htab->num_buf << htab->line_size_log2, bfd_abs_section_ptr},
            {"__icache_linesize", 1U << htab->line_size_log2, 0, bfd_abs_section_ptr},
            {"__icache_log2_linesize", htab->line_size_log2, 0, bfd_abs_section_ptr},
            {"__icache_neg_log2_linesize", (bfd_vma)(-(int)htab->line_size_log2), 0, bfd_abs_section_ptr},
            {"__icache_cachesize", 1U << (htab->num_lines_log2 + htab->line_size_log2), 0, bfd_abs_section_ptr},
            {"__icache_log2_cachesize", htab->num_lines_log2 + htab->line_size_log2, 0, bfd_abs_section_ptr},
            {"__icache_neg_log2_cachesize", (bfd_vma)(-(int)(htab->num_lines_log2 + htab->line_size_log2)), 0, bfd_abs_section_ptr},
        };

        size_t num_syms = sizeof(syms) / sizeof(syms[0]);
        size_t idx = 0;

        h = define_ovtab_symbol(htab, syms[idx].name);
        if (!h) return false;
        h->root.u.def.value = syms[idx].value;
        h->size = syms[idx].size;
        idx++;
        off = h->size;

        for (; idx < num_syms; idx++) {
            h = define_ovtab_symbol(htab, syms[idx].name);
            if (!h) return false;
            h->root.u.def.value = syms[idx].name[9] == 't' && syms[idx].name[10] == 'o' ? off : syms[idx].value;
            if (!syms[idx].section) {
                if (syms[idx].name[9] == 't' && syms[idx].name[10] == 'o') {
                    h->size = syms[idx].size;
                    off += h->size;
                } else if (syms[idx].name[9] == 'f' && syms[idx].name[10] == 'r') {
                    h->size = syms[idx].size;
                    off += h->size;
                }
            }
            h->root.u.def.section = syms[idx].section;
        }
        ...
        if (htab->ovtab->size > 7)
            p[7] = 1;

        obfd = htab->ovtab->output_section ? htab->ovtab->output_section->owner : NULL;
        for (s = obfd ? obfd->sections : NULL; s != NULL; s = s->next) {
            ...
            if (ovl_index != 0) {
                ...
                bfd_put_32(htab->ovtab->owner, (s->size + 15) & (bfd_vma)~0xFULL, p + off + 4);
        ...
        struct {
            ...
            {"_ovly_table", 16, htab->num_overlays * 16},
            {"_ovly_table_end", htab->num_overlays * 16 + 16, 0},
            {"_ovly_buf_table", htab->num_overlays * 16 + 16, htab->num_buf * 4},
            {"_ovly_buf_table_end", htab->num_overlays * 16 + 16 + htab->num_buf * 4, 0},
        };

        for (i = 0; i < (sizeof(ov_syms)/sizeof(ov_syms[0])); i++) {
            h = define_ovtab_symbol(htab, ov_syms[i].name);
            if (!h) return false;
            h->root.u.def.value = ov_syms[i].value;
            h->size = ov_syms[i].size;
        }
    }
...
asection *spu_elf_check_vma(struct bfd_link_info *info)
{
    ...
    struct spu_link_hash_table *htab;
    bfd *abfd;
    bfd_vma hi, lo, vma, size, end_vma;
    ...
    for (m = elf_seg_map(abfd); m != NULL; m = m->next) {
        ...
        for (i = 0; i < m->count; i++) {
            ...
            if (size > (bfd_vma)-1 - vma) {
                return sec;
            }
            end_vma = vma + size - 1;
            if (vma < lo || vma > hi || end_vma > hi) {
                return sec;
            }
        }
    }
...
static int sort_syms(const void *a, const void *b)
{
    ...
    ptrdiff_t idx1 = *s1 - sort_syms_syms;
    ptrdiff_t idx2 = *s2 - sort_syms_syms;

    if (idx1 < 0 || idx2 < 0)
        return 0;

    asection *sec1 = sort_syms_psecs[idx1];
    asection *sec2 = sort_syms_psecs[idx2];

    if (sec1 != sec2) {
        if (sec1->index < sec2->index) return -1;
        if (sec1->index > sec2->index) return 1;
    }

    if ((*s1)->st_value < (*s2)->st_value) return -1;
    if ((*s1)->st_value > (*s2)->st_value) return 1;
...
alloc_stack_info(asection *sec, int max_fun)
{
    ...
    if (sec_data == NULL || max_fun <= 0)
        return NULL;

    amt = sizeof(struct spu_elf_stack_info) + (max_fun - 1) * sizeof(struct function_info);
...
maybe_insert_function(asection *sec, void *sym_h, bool global, bool is_func)
{
    ...
    if (sinfo->num_fun >= sinfo->max_fun) {
        bfd_size_type amt = sizeof(struct spu_elf_stack_info) + (sinfo->max_fun - 1) * sizeof(struct function_info);
        bfd_size_type new_max_fun = sinfo->max_fun + 20 + (sinfo->max_fun >> 1);
        bfd_size_type new_amt = sizeof(struct spu_elf_stack_info) + (new_max_fun - 1) * sizeof(struct function_info);
        struct spu_elf_stack_info *new_sinfo = bfd_realloc(sinfo, new_amt);
        if (!new_sinfo)
            return NULL;
        memset((char *)new_sinfo + amt, 0, new_amt - amt);
        new_sinfo->max_fun = new_max_fun;
        sinfo = new_sinfo;
        sec_data->u.i.stack_info = sinfo;
    }
    ...
    sinfo->fun[i].sec = sec;
    sinfo->fun[i].u.h = global ? sym_h : NULL;
    sinfo->fun[i].u.sym = global ? NULL : sym_h;
    sinfo->fun[i].lo = off;
    sinfo->fun[i].hi = off + size;
    sinfo->fun[i].lr_store = -1;
    sinfo->fun[i].sp_adjust = -1;
    sinfo->fun[i].stack = -find_function_stack_adjust(sec, off, &sinfo->fun[i].lr_store, &sinfo->fun[i].sp_adjust);
...
func_name(struct function_info *fun)
{
    ...
    if (fun->global && fun->u.h && fun->u.h->root.root.string)
        return fun->u.h->root.root.string;

    if (fun->sec == NULL || fun->u.sym == NULL)
        return "(null)";

    if (fun->u.sym->st_name == 0) {
        size_t len = strlen(fun->sec->name);
        char *name = bfd_malloc(len + 11);
        if (name == NULL)
            return "(null)";
        snprintf(name, len + 11, "%s+%lx", fun->sec->name,
                 (unsigned long)(fun->u.sym->st_value & 0xffffffff));
        return name;
    }

    if (fun->sec->owner == NULL)
        return "(null)";

    {
        bfd *ibfd = fun->sec->owner;
        const struct elf_backend_data *elf_data = elf_tdata(ibfd);
        if (elf_data == NULL)
            return "(null)";
        Elf_Internal_Shdr *symtab_hdr = &elf_data->symtab_hdr;
        return bfd_elf_sym_name(ibfd, symtab_hdr, fun->u.sym, fun->sec);
    }
}
...
static bool is_nop(asection *sec, bfd_vma off)
{
    unsigned char insn[4];

    if (!sec || !sec->owner || off > sec->size || sec->size - off < 4)
        return false;
    ...
    if (((insn[0] & 0xbf) == 0 && (insn[1] & 0xe0) == 0x20) ||
        (insn[0] == 0 && insn[1] == 0 && insn[2] == 0 && insn[3] == 0))
        return true;
...
static bool check_function_ranges(asection *sec, struct bfd_link_info *info)
{
    ...
    if (sec == NULL || info == NULL || info->callbacks == NULL || sec->size == 0)
        return false;
...
find_function(asection *sec, bfd_vma offset, struct bfd_link_info *info)
{
    ...
    if (!sinfo || !sinfo->fun || sinfo->num_fun <= 0) {
        bfd_set_error(bfd_error_bad_value);
        return NULL;
    }
...
static bool copy_callee(struct function_info *caller, const struct call_info *call) {
...
static bool interesting_section(asection *s) {
  if (!s || !s->output_section || s->output_section == bfd_abs_section_ptr) {
    return false;
  }
  if ((s->flags & (SEC_ALLOC | SEC_LOAD | SEC_CODE | SEC_IN_MEMORY)) != (SEC_ALLOC | SEC_LOAD | SEC_CODE)) {
    return false;
  }
  if (s->size == 0) {
    return false;
...
static bool mark_functions_via_relocs(asection *sec, struct bfd_link_info *info, int call_tree)
{
    ...
    for (; irela < irelaend; irela++) {
        ...
        if (!call_tree) {
            ...
            Elf_Internal_Sym *allocated_fake = NULL;

            if (irela->r_addend != 0) {
                allocated_fake = bfd_zmalloc(sizeof(*allocated_fake));
                if (!allocated_fake)
                    return false;
                allocated_fake->st_value = val;
                allocated_fake->st_shndx = _bfd_elf_section_from_bfd_section(sym_sec->owner, sym_sec);
                sym = allocated_fake;
            }
            ...
            if (allocated_fake && fun->u.sym != allocated_fake)
                free(allocated_fake);
...
discover_functions(struct bfd_link_info *info)
{
    ...
    psym_arr = bfd_zmalloc(bfd_count * sizeof(*psym_arr));
    if (!psym_arr)
        return false;

    sec_arr = bfd_zmalloc(bfd_count * sizeof(*sec_arr));
    if (!sec_arr) {
        free(psym_arr);
        return false;
    }
    ...
    for (ibfd = info->input_bfds, bfd_idx = 0; ibfd != NULL; ibfd = ibfd->link.next, bfd_idx++) {
        ...
        asection *sec;
        size_t symcount;
        Elf_Internal_Sym *syms = NULL, *sy, **psyms = NULL, **psy;
        asection **psecs = NULL, **p;

        psym_arr[bfd_idx] = NULL;
        sec_arr[bfd_idx] = NULL;
        ...
        *psy = NULL;
        ...
        for (psy = psyms; psy < psyms + symcount;) {
            ...
            for (psy2 = psy + 1; psy2 < psyms + symcount; ++psy2)
...
cleanup_and_fail:
    for (ibfd = info->input_bfds, bfd_idx = 0; bfd_idx < bfd_count; ibfd = (ibfd ? ibfd->link.next : NULL), bfd_idx++) {
        if (psym_arr && psym_arr[bfd_idx]) {
            free(psym_arr[bfd_idx]);
            psym_arr[bfd_idx] = NULL;
        }
        if (sec_arr && sec_arr[bfd_idx]) {
            free(sec_arr[bfd_idx]);
            sec_arr[bfd_idx] = NULL;
        }
    }
    ...
    for (bfd *ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next) {
        ...
        for (asection *sec = ibfd->sections; sec != NULL; sec = sec->next) {
            ...
            for (int i = 0; i < sinfo->num_fun; ++i) {
                struct function_info *finfo = &sinfo->fun[i];
                if (root_only && finfo->non_root)
                    continue;
                if (!doit(finfo, info, param))
                    return false;
            }
        }
    }
...
static bool transfer_calls(struct function_info *fun, struct bfd_link_info *info ATTRIBUTE_UNUSED, void *param ATTRIBUTE_UNUSED)
{
    if (fun == NULL)
        return false;
...
static bool mark_non_root(struct function_info *fun, struct bfd_link_info *info, void *param) {
...
static bool remove_cycles(struct function_info *fun, struct bfd_link_info *info, void *param) {
    if (!fun || !info || !param)
        return false;
    ...
    while (*callp) {
        struct call_info *call = *callp;
        call->max_depth = depth + (call->is_pasted ? 0 : 1);
        ...
        if (!callee->visit2) {
            if (!remove_cycles(callee, info, &call->max_depth))
                return false;
            if (max_depth < call->max_depth)
                max_depth = call->max_depth;
        } else if (callee->marking) {
            struct spu_link_hash_table *htab = spu_hash_table(info);
            if (!htab || !htab->params)
                return false;
            ...
            if (!htab->params->auto_overlay && htab->params->stack_analysis) {
                ...
                const char *f2 = func_name(callee);
                if (info->callbacks && info->callbacks->info && f1 && f2) {
                    info->callbacks->info(
                        _("stack analysis will ignore the call from %s to %s\n"),
                        f1, f2
                    );
                } else {
                    return false;
...
static bool mark_detached_root(struct function_info *fun, struct bfd_link_info *info, void *param) {
...
static bool build_call_tree(struct bfd_link_info *info) {
    if (info == NULL || info->input_bfds == NULL)
        return false;
    ...
    struct spu_link_hash_table *hash_table = spu_hash_table(info);
    if (hash_table == NULL || hash_table->params == NULL)
        return false;

    if (!hash_table->params->auto_overlay) {
...
static bool mark_overlay_section(struct function_info *fun, struct bfd_link_info *info, void *param)
{
        ...
        if (htab->params->auto_overlay & OVERLAY_RODATA) {
            ...
            if (name) {
                ...
                if (fun->rodata) {
                    ...
                    if (htab->params->line_size && size > htab->params->line_size) {
                        size -= fun->rodata->size;
                        fun->rodata = NULL;
                    } else {
    ...
    for (call = fun->call_list; call != NULL; call = call->next) {
        ...
        if (!call->broken_cycle && !mark_overlay_section(call->fun, info, param))
            return false;
    }
...
static bool
unmark_overlay_section(struct function_info *fun, struct bfd_link_info *info, void *param)
{
    struct call_info *call;
    ...
    excluded = (fun->sec == uos_param->exclude_input_section ||
                (fun->sec && fun->sec->output_section == uos_param->exclude_output_section)) ? 1 : 0;
...
static bool collect_lib_sections(struct function_info *fun, struct bfd_link_info *info, void *param) {
    struct _cl_param *lib_param = param;
    struct call_info *call;
    unsigned int size;
    bool rodata_included = false;
    ...
    if (!fun->sec || !fun->sec->linker_mark || !fun->sec->gc_mark || fun->sec->segment_mark)
        return true;
...
static int sort_lib(const void *a, const void *b)
{
    ...
    struct _spu_elf_section_data *sec_data1 = spu_elf_section_data(*s1);
    if (sec_data1 && sec_data1->u.i.stack_info) {
        struct spu_elf_stack_info *sinfo1 = sec_data1->u.i.stack_info;
        for (int i = 0; i < sinfo1->num_fun; ++i) {
            sum1 += sinfo1->fun[i].call_count;
        }
    }

    struct _spu_elf_section_data *sec_data2 = spu_elf_section_data(*s2);
    if (sec_data2 && sec_data2->u.i.stack_info) {
        struct spu_elf_stack_info *sinfo2 = sec_data2->u.i.stack_info;
        for (int i = 0; i < sinfo2->num_fun; ++i) {
            sum2 += sinfo2->fun[i].call_count;
        }
    }
...
static unsigned int auto_ovl_lib_functions(struct bfd_link_info *info, unsigned int lib_size)
{
    ...
    for (i = 0; i < lib_count; i++) {
        unsigned int total_size, stub_size = 0;
        ...
        if (lib_sections[2 * i + 1])
            total_size += lib_sections[2 * i + 1]->size;

        sec_data = spu_elf_section_data(sec);
        if (total_size < lib_size && sec_data != NULL && (sinfo = sec_data->u.i.stack_info) != NULL) {
            int k;
            for (k = 0; k < sinfo->num_fun; ++k) {
                struct call_info *call;
                for (call = sinfo->fun[k].call_list; call; call = call->next) {
                    if (call->fun->sec->linker_mark) {
                        struct call_info *p = dummy_caller.call_list;
                        while (p && p->fun != call->fun)
                            p = p->next;
                        if (!p)
        ...
        if (total_size + stub_size >= lib_size)
            continue;
        ...
        lib_size -= total_size + stub_size;

        struct call_info **pp = &dummy_caller.call_list, *p;
        while ((p = *pp) != NULL) {
        ...
        sec_data = spu_elf_section_data(sec);
        sinfo = (sec_data != NULL) ? sec_data->u.i.stack_info : NULL;
        if (sinfo != NULL) {
            int k;
            for (k = 0; k < sinfo->num_fun; ++k) {
                struct call_info *call;
                    ...
                    if (call->fun->sec->linker_mark) {
                        ...
                        if (callee == NULL) {
                            while (dummy_caller.call_list) {
                                struct call_info *tmp = dummy_caller.call_list;
                                dummy_caller.call_list = tmp->next;
                                free(tmp);
                            }
                            free(lib_sections);
                            return (unsigned int)-1;
...
static bool
sum_stack(struct function_info *fun, struct bfd_link_info *info, void *param)
{
    struct call_info *call;
    struct function_info *max_fun = NULL;
    ...
    if (fun == NULL || info == NULL || sum_stack_param == NULL)
        return false;
    ...
    func1_name = func_name(fun);
    if (htab->params->stack_analysis) {
        if (!fun->non_root)
            info->callbacks->info("  %s: 0x%v\n", func1_name, (bfd_vma)cum_stack);
        info->callbacks->minfo("%s: 0x%v 0x%v\n", func1_name, (bfd_vma)stack, (bfd_vma)cum_stack);

        if (has_call) {
            info->callbacks->minfo(_("  calls:\n"));
            for (call = fun->call_list; call != NULL; call = call->next) {
                if (!call->is_pasted && !call->broken_cycle) {
                    const char *func2_name = func_name(call->fun);
                    const char *ann1 = (call->fun == max_fun) ? "*" : " ";
                    const char *ann2 = call->is_tail ? "t" : " ";

                    info->callbacks->minfo("   %s%s %s\n", ann1, ann2, func2_name);
                }
            }
        }
    }
    ...
    if (sum_stack_param->emit_stack_syms) {
        ...
        name_len = 32 + strlen(func1_name);
        name = (char *)bfd_malloc(name_len);
        if (name == NULL)
            return false;

        if (fun->global || (fun->u.sym && ELF_ST_BIND(fun->u.sym->st_info) == STB_GLOBAL))
            snprintf(name, name_len, "__stack_%s", func1_name);
        else if (fun->sec)
            snprintf(name, name_len, "__stack_%x_%s", (unsigned)(fun->sec->id & 0xffffffff), func1_name);
        else
            snprintf(name, name_len, "__stack_x_%s", func1_name);
...
static unsigned int print_one_overlay_section(FILE *script,
                                             unsigned int base,
                                             unsigned int count,
                                             ...
                                             struct bfd_link_info *info)
{
    ...
    if (!script || !ovly_map || !ovly_sections || !info)
        return 0;
    ...
    for (j = base; j < count && ovly_map[j] == ovlynum; ++j) {
        ...
        if (sec->segment_mark) {
            ...
            while (call != NULL) {
                ...
                sec = (call_fun && call_fun->sec) ? call_fun->sec : NULL;
                if (!sec || !sec->owner)
                    return (unsigned int)-1;
    ...
    for (j = base; j < count && ovly_map[j] == ovlynum; ++j) {
        ...
        if (sec && sec->segment_mark) {
            struct call_info *call = find_pasted_call(sec);
            while (call != NULL) {
                struct function_info *call_fun = call->fun;
                asection *rodata = (call_fun) ? call_fun->rodata : NULL;
                if (rodata && rodata->owner) {
                    ret = fprintf(script, "   %s%c%s (%s)\n",
                                  (rodata->owner->my_archive != NULL
                                      ? bfd_get_filename(rodata->owner->my_archive) : ""),
                                  info->path_separator,
                                  bfd_get_filename(rodata->owner),
...
static void spu_elf_auto_overlay(struct bfd_link_info *info)
{
    ...
    bfd **bfd_arr = NULL;
    struct elf_segment_map *m;
    unsigned int fixed_size = 0, lo, hi;
    unsigned int reserved;
    struct spu_link_hash_table *htab;
    unsigned int base, i, count, bfd_count;
    unsigned int region, ovlynum;
    asection **ovly_sections = NULL, **ovly_p = NULL;
            ...
            for (i = 0; i < m->count; i++)
                if (m->sections[i]->size != 0) {
                    if (m->sections[i]->vma < lo)
                        lo = m->sections[i]->vma;
                    unsigned int end = m->sections[i]->vma + m->sections[i]->size - 1;
                    if (end > hi)
                        hi = end;
                }
    fixed_size = hi + 1 - lo;
    ...
    if (!error && fixed_size + reserved <= htab->local_store &&
        htab->params->ovly_flavour != ovly_soft_icache) {
    ...
    if (!error && !for_each_node(mark_overlay_section, info, &mos_param, true))
        error = true;

    uos_param.clearing = 0;
    if (!error
        && (uos_param.exclude_input_section || uos_param.exclude_output_section)
    ...
    for (ibfd = info->input_bfds; !error && ibfd != NULL; ibfd = ibfd->link.next) {
        extern const bfd_target spu_elf32_vec;
        asection *sec;
    ...
    if (!error && fixed_size + mos_param.max_overlay_size <= htab->local_store) {
        if (htab->params->ovly_flavour == ovly_soft_icache) {
    ...
    if (!error && fixed_size + mos_param.max_overlay_size > htab->local_store) {
        ...
        error = true;
    } else if (!error && fixed_size < htab->params->auto_overlay_fixed) {
                ...
                if (sec->segment_mark) {
                    ...
                    while (pasty != NULL) {
                        ...
                        tmp = align_power(tmp, call_fun->sec->alignment_power)
                            + call_fun->sec->size;
                        if (call_fun->rodata) {
                            rotmp = align_power(rotmp, call_fun->rodata->alignment_power)
    ...
    if (!error && htab->params->ovly_flavour == ovly_soft_icache) {
        ...
        if (!error &&
            (fprintf(script, " . = ABSOLUTE (ADDR (.ovl.init)) + %u;\n",
        ...
        if (fprintf(script, "SECTIONS\n{\n") <= 0
            || fprintf(script, " . = ALIGN (16);\n .ovl.init : { *(.ovl.init) }\n . = ABSOLUTE (ADDR (.ovl.init));\n") <= 0) {
            bfd_set_error(bfd_error_system_call);
            error = true;
        }
        for (region = 1; !error && region <= htab->params->num_lines; region++) {
            ovlynum = region;
            base = 0;
    ...
    if (!error && fclose(script) != 0) {
        bfd_set_error(bfd_error_system_call);
        error = true;
    }

    if (!error && (htab->params->auto_overlay & AUTO_RELINK))
...
static bool spu_elf_stack_analysis(struct bfd_link_info *info)
{
    struct spu_link_hash_table *htab;
    struct _sum_stack_param sum_stack_param;
    ...
    if (htab->params->stack_analysis) {
        if (info->callbacks && info->callbacks->info && info->callbacks->minfo) {
...
static bool spu_elf_final_link(bfd *output_bfd, struct bfd_link_info *info)
{
    ...
    if (!htab || !htab->params || !info || !info->callbacks)
        return false;
...
static void spu_elf_emit_fixup(bfd *output_bfd, struct bfd_link_info *info, bfd_vma offset)
{
    ...
    if (sfixup->reloc_count == 0) {
        if (FIXUP_RECORD_SIZE > sfixup->size) {
    ...
    bfd_vma last_record_index = sfixup->reloc_count - 1;
    bfd_vma base = FIXUP_GET(output_bfd, htab, last_record_index);
    ...
    FIXUP_PUT(output_bfd, htab, last_record_index, base | bit);
}

/* Apply RELOCS to CONTENTS of INPUT_SECTION from INPUT_BFD.  */

static int spu_elf_relocate_section(
    bfd *output_bfd,
    struct bfd_link_info *info,
    bfd *input_bfd,
    asection *input_section,
    ...
    for (; rel < relend; rel++) {
        ...
        if (r_symndx < symtab_hdr->sh_info) {
            ...
            if (h && (h->root.type == bfd_link_hash_defined || h->root.type == bfd_link_hash_defweak)) {
                ...
                bool err = (info->unresolved_syms_in_objects == RM_DIAGNOSE && !info->warn_unresolved_syms)
                           || (h && ELF_ST_VISIBILITY(h->other) != STV_DEFAULT);

                info->callbacks->undefined_symbol(
                    info,
                    h ? h->root.root.string : NULL,
                    input_bfd,
                    input_section,
                    rel->r_offset,
                    err);
            ...
            sym_name = h ? h->root.root.string : NULL;
        }

        if (sec && discarded_section(sec)) {
            RELOC_AGAINST_DISCARDED_SECTION(
                info, input_bfd, input_section, rel, 1, relend, R_SPU_NONE, howto, 0, contents);
            continue;
        }
        ...
        addend = rel->r_addend;
        if (stubs && !is_ea_sym &&
            (stub_type = needs_ovl_stub(h, sym, sec, input_section, rel, contents, info)) != no_stub) {
            unsigned int ovl = 0;
            struct got_entry *g = NULL, **head = NULL;
            ...
            for (g = (head ? *head : NULL); g; g = g->next) {
                if (htab->params->ovly_flavour == ovly_soft_icache) {
                    if (g->ovl == ovl && g->br_addr == (rel->r_offset + input_section->output_offset +
        ...
        if (htab->params->emit_fixups && !bfd_link_relocatable(info) &&
            (input_section->flags & SEC_ALLOC) && r_type == R_SPU_ADDR32) {
            bfd_vma offset = rel->r_offset + input_section->output_section->vma +
                             input_section->output_offset;
            spu_elf_emit_fixup(output_bfd, info, offset);
        }

        if (!unresolved_reloc && (r_type == R_SPU_PPU32 || r_type == R_SPU_PPU64)) {
            if (is_ea_sym) {
        ...
        if (unresolved_reloc &&
            ...
            _bfd_error_handler(
                _("%pB(%s+%#" PRIx64 "): unresolvable %s relocation against symbol `%s'"),
                input_bfd, bfd_section_name(input_section), (uint64_t)rel->r_offset, howto->name,
                sym_name);
            ret = 0;
        }

        r = _bfd_final_link_relocate(howto, input_bfd, input_section, contents, rel->r_offset, relocation, addend);
        ...
        if (r != bfd_reloc_ok) {
            ...
            case bfd_reloc_overflow:
                info->callbacks->reloc_overflow(
                    info, (h ? &h->root : NULL), sym_name, howto->name, (bfd_vma)0, input_bfd, input_section,
                    rel->r_offset);
                break;
            case bfd_reloc_undefined:
                info->callbacks->undefined_symbol(
                    info, sym_name, input_bfd, input_section, rel->r_offset, 1);
                break;
...
static bool spu_elf_finish_dynamic_sections(bfd *output_bfd, struct bfd_link_info *info)
{
    (void)output_bfd;
    (void)info;
    return true;
...
static int spu_elf_output_symbol_hook(struct bfd_link_info *info,
    ...
    if (!bfd_link_relocatable(info) &&
        ...
        for (struct got_entry *g = h->got.glist; g != NULL; g = g->next) {
            int match = 0;
            if (htab->params && htab->params->ovly_flavour == ovly_soft_icache) {
                match = (g->br_addr == g->stub_addr);
            } else {
                match = (g->addend == 0 && g->ovl == 0);
            }
            if (match && htab->stub_sec[0] && htab->stub_sec[0]->output_section &&
...
static bool spu_elf_init_file_header(bfd *abfd, struct bfd_link_info *info)
{
...
{
    asection *toe, *s;
    struct elf_segment_map *m, *m_overlay = NULL;
    struct elf_segment_map **p, **p_overlay, **first_load = NULL;
    unsigned int i;
        ...
        if (m->p_type == PT_LOAD && m->count > 1) {
            for (i = 0; i < m->count; i++) {
                ...
                if (s == toe || spu_elf_section_data(s)->u.o.ovl_index != 0) {
                    ...
                    if (i + 1 < m->count) {
                        amt = sizeof(struct elf_segment_map) + (m->count - (i + 2)) * sizeof(m->sections[0]);
    ...
    if (m_overlay != NULL) {
        p = first_load ? first_load : &elf_seg_map(abfd);
...
static bool spu_elf_fake_sections(bfd *obfd ATTRIBUTE_UNUSED, Elf_Internal_Shdr *hdr, asection *sec) {
...
spu_elf_modify_headers(bfd *abfd, struct bfd_link_info *info)
{
    ...
    if (htab && htab->num_overlays != 0) {
        struct elf_segment_map *m = elf_seg_map(abfd);
        unsigned int i = 0;

        while (m) {
            if (m->count != 0) {
                unsigned int o = spu_elf_section_data(m->sections[0])->u.o.ovl_index;
                if (o != 0) {
                    phdr[i].p_flags |= PF_OVERLAY;
                    if (htab->ovtab && htab->ovtab->size != 0 &&
                        htab->params && htab->params->ovly_flavour != ovly_soft_icache) {
    ...
    for (int idx = count - 1; idx >= 0; --idx) {
        ...
        unsigned int adjust_filesz = ((~phdr[idx].p_filesz) + 1) & 15;
        if (adjust_filesz != 0 && last != NULL &&
            phdr[idx].p_offset + phdr[idx].p_filesz > last->p_offset - adjust_filesz) {
            need_adjust = 0;
            break;
        }

        unsigned int adjust_memsz = ((~phdr[idx].p_memsz) + 1) & 15;
        if (adjust_memsz != 0 && last != NULL &&
            phdr[idx].p_filesz != 0 &&
            phdr[idx].p_vaddr + phdr[idx].p_memsz > last->p_vaddr - adjust_memsz &&
            phdr[idx].p_vaddr + phdr[idx].p_memsz <= last->p_vaddr) {
            need_adjust = 0;
            break;
        }

        if (phdr[idx].p_filesz != 0)
            last = &phdr[idx];
    }
    ...
    if (need_adjust) {
        for (int idx = count - 1; idx >= 0; --idx) {
            if (phdr[idx].p_type == PT_LOAD) {
                unsigned int adjust_filesz = ((~phdr[idx].p_filesz) + 1) & 15;
                phdr[idx].p_filesz += adjust_filesz;

                unsigned int adjust_memsz = ((~phdr[idx].p_memsz) + 1) & 15;
                phdr[idx].p_memsz += adjust_memsz;
            }
        }
    }
    ...
    for (bfd *ibfd = info->input_bfds; ibfd; ibfd = ibfd->link.next) {
        ...
        for (asection *isec = ibfd->sections; isec; isec = isec->next) {
            ...
            Elf_Internal_Rela *internal_relocs = _bfd_elf_link_read_relocs(
                ibfd, isec, NULL, NULL, info->keep_memory);
